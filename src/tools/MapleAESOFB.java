...
public class MapleAESOFB
{
        ...
        try {
            while (remaining > 0) {
                ...
                for (int x = start; x < start + llength; ++x) {
                    ...
                    final int tmp98_96 = x;
                    final byte[] tmp98_97 = data;
                    tmp98_97[tmp98_96] ^= myIv[(x - start) % myIv.length];
                }
                start += llength;
                remaining -= llength;
                llength = 1460;
    ...
    public static final void funnyShit(final byte inputByte, final byte[] in) {
        ...
        moritz -= inputByte;
        final int tmp26_25 = 0;
        final byte[] tmp26_26 = in;
        tmp26_26[tmp26_25] += moritz;
        moritz = in[2];
