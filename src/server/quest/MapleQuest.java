...
public class MapleQuest implements Serializable
{
    ...
    private static final List<MapleQuest> BulbQuest;
    ...
    public static MapleQuest getInstance(final int id) {
        MapleQuest ret = (server.quest.MapleQuest)(server.quest.MapleQuest)MapleQuest.quests.get(id);
        if (ret == null) {
            ...
            try {
                ...
                FileoutputUtil.log("日志/Logs/Log_Script_脚本异常.rtf", "Caused by questID " + id);
    ...
    public void complete(final MapleCharacter c, final int npc, final Integer selection) {
        if ((this.autoPreComplete || this.checkNPCOnMap(c, npc)) && this.canComplete(c, npc)) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.showOwnBuffEffect(0, 10));
            c.getMap().broadcastMessage(c, MaplePacketCreator.showBuffeffect(c.getId(), 0, 10, (byte)3), false);
        }
        else {
            if (npc != 9010000) {
                for (final MapleQuestAction a : this.completeActs) {
    ...
    static {
        ...
        BulbQuest = new LinkedList<MapleQuest>();
    }
