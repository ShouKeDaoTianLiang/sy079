...
{
    protected Map<Integer, Pair<Integer, List<RewardItem>>> rewardCache;
    ...
    protected final Map<Integer, String> hairList;
    ...
    protected MapleItemInformationProvider() {
        this.rewardCache = new HashMap<Integer, Pair<Integer, List<RewardItem>>>();
        ...
        this.nameCache = new ConcurrentHashMap<Integer, String>();
        ...
        this.hairList = new HashMap<Integer, String>();
    }
    ...
    public final List<Pair<Integer, String>> getAllItems() {
        ...
        return (List<Pair<Integer, String>>)itemPairs;
    }
    ...
    public int getScrollSuccess(final int itemId) {
        if (itemId / 10000 != 204 || this.getEquipStats(itemId) == null || !this.getEquipStats(itemId).containsKey("success")) {
            return 0;
        }
        return this.getEquipStats(itemId).get("success");
    }
    
    public final Item scrollEquipWithId(final Item equip, final Item scrollId, final boolean ws, final MapleCharacter chr, final int vegas, final boolean checkIfGM) {
        if (equip.getType() == 1) {
            final Equip nEquip = (Equip)equip;
            final MapleItemInformationProvider ii = getInstance();
            ...
            if (chr.isAdmin()) {
                chr.dropMessage(5, "装备:[" + ii.getName(equip.getItemId()) + "], 普通卷轴 - 默认几率: " + succ + "% 最终概率: " + success + "% 失败消失几率: " + curse + "%");
            }
    ...
    public final Item getEquipById(final int equipId, final int ringId) {
        final Equip nEquip = new Equip(equipId, (short)0, ringId, (short)0);
    ...
    public final MapleStatEffect getItemEffect(final int itemId) {
        MapleStatEffect ret = (server.MapleStatEffect)(server.MapleStatEffect)this.itemEffects.get(itemId);
        if (ret == null) {
            final MapleData item = this.getItemData(itemId);
            if (item == null) {
                return null;
    ...
    public final List<Pair<Integer, Integer>> getSummonMobs(final int itemId) {
        ...
        final List mobPairs = new ArrayList();
        ...
        return (List<Pair<Integer, Integer>>)mobPairs;
    }
    ...
    public final int getWatkForProjectile(final int itemId) {
        Integer atk = (Integer)(Integer)this.projectileWatkCache.get(itemId);
        if (atk != null) {
            return atk;
        }
    ...
    public final Pair<Integer, List<StructRewardItem>> getRewardItem(final int itemid) {
        ...
        for (final MapleData reward : rewards) {
            ...
            struct.prob = (short)MapleDataTool.getInt("prob", reward, 0);
        ...
        final Pair toreturn = new Pair(totalprob, all);
        this.RewardItem.put(itemid, toreturn);
        return (Pair<Integer, List<StructRewardItem>>)toreturn;
    }
    ...
    public final Pair<Integer, List<Integer>> questItemInfo(final int itemId) {
        ...
        final List consumeItems = new ArrayList();
        for (final MapleData consume : itemD.getChildByPath("consumeItem")) {
            consumeItems.add(MapleDataTool.getInt(consume, 0));
        }
        final Pair questItem = new Pair(MapleDataTool.getIntConvert("questId", itemD, 0), consumeItems);
        this.questItems.put(itemId, questItem);
        return (Pair<Integer, List<Integer>>)questItem;
    }
    ...
    public Pair<Integer, List<RewardItem>> getItemReward(final int itemId) {
        if (this.rewardCache.containsKey(itemId)) {
            return this.rewardCache.get(itemId);
        }
        int totalprob = 0;
        final List<RewardItem> rewards = new ArrayList<RewardItem>();
        for (final MapleData child : this.getItemData(itemId).getChildByPath("reward").getChildren()) {
            final RewardItem reward = new RewardItem();
            reward.itemid = MapleDataTool.getInt("item", child, 0);
            reward.prob = (short)MapleDataTool.getInt("prob", child, 0);
        ...
        final Pair<Integer, List<RewardItem>> hmm = new Pair<Integer, List<RewardItem>>(totalprob, rewards);
    ...
    public List<Pair<String, Integer>> getItemLevelupStats(final int itemId, final int level, final boolean timeless) {
        ...
        return (List<Pair<String, Integer>>)list;
    }
