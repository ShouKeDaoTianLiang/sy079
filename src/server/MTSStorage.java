...
public class MTSStorage
{
    ...
    private void loadBuyNow() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final Map<Integer, Pair<Item, MapleInventoryType>> items = ItemLoader.MTS.loadItems(false, lastPackage);
                if (items != null && items.size() > 0) {
                    for (final Pair<Item, MapleInventoryType> i : items.values()) {
    ...
    public final void saveBuyNow(final boolean isShutDown) {
        ...
        try {
            for (final Entry<Integer, ArrayList<Pair<Item, MapleInventoryType>>> ite : items.entrySet()) {
        ...
        try {
            for (final Entry<Integer, MTSCart> c : this.idToCart.entrySet()) {
                ...
                if (expire.containsKey(c.getKey())) {
                    for (final Item item : expire.get(c.getKey())) {
                        c.getValue().addToInventory(item);
                    }
                }
                c.getValue().save();
    ...
    public final MTSCart getCart(final int characterId) {
        ...
        MTSCart ret;
        try {
            ret = (server.MTSCart)(server.MTSCart)this.idToCart.get(characterId);
        }
        finally {
            this.cart_mutex.readLock().unlock();
        }
    ...
    public static class MTSItemInfo
    {
        ...
        public MTSItemInfo(final int price, final Item item, final String seller, final int id, final int cid, final long date) {
            this.item = item;
            this.price = price;
            this.seller = seller;
            this.id = id;
