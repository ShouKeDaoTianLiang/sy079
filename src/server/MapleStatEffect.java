...
public class MapleStatEffect implements Serializable
{
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final byte level) {
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case 1311008: {
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.DRAGONBLOOD, (int)ret.watk));
                    break;
                }
                case 1121000:
                case 1221000:
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, final int newDuration) {
        ...
        if (this.expinc != 0) {
            ...
            for (int i = 0; i < use.getSlotLimit(); ++i) {
                final Item item = use.getItem((short)i);
        ...
        if (summonMovementType != null) {
            ...
            if (door.getTownPortal() != null) {
                ...
                applyto.dropMessage(5, "你可能办法使用传送门村庄内禁止..");
            }
        }
        else if (this.isMist()) {
    ...
    public Rectangle calculateBoundingBox(final Point posFrom, final boolean facingLeft, final int addedRange) {
        return calculateBoundingBox(posFrom, facingLeft, this.lt, this.rb, addedRange);
    }
    ...
    public final Rectangle calculateBoundingBox(final Point posFrom, final boolean facingLeft, final boolean isLeft) {
        if (this.lt == null || this.rb == null) {
            return new Rectangle(posFrom.x, posFrom.y, facingLeft ? 1 : -1, 1);
        }
        Point mylt;
    ...
    public final void silentApplyBuff(final MapleCharacter chr, final long starttime) {
        ...
        chr.registerEffect(this, starttime, BuffTimer.getInstance().schedule(new CancelEffectAction(chr, this, starttime), starttime + localDuration - System.currentTimeMillis()));
        final SummonMovementType summonMovementType = this.getSummonMovementType();
        if (summonMovementType != null) {
    ...
    public final void applyComboBuffA(final MapleCharacter applyto, final short combo) {
        ...
        final ScheduledFuture<?> schedule = BuffTimer.getInstance().schedule(cancelAction, starttime + 29999L - System.currentTimeMillis());
        applyto.registerEffect(this, starttime, schedule, statups);
    }
    
    public final void applyComboBuff4(final MapleCharacter applyto, final short combo) {
        final ArrayList<Pair<MapleBuffStat, Integer>> statups = new ArrayList<Pair<MapleBuffStat, Integer>>();
        statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SHARP_EYES, this.x << 8 | this.y));
        applyto.getClient().getSession().write(MaplePacketCreator.giveBuff(21110000, 120000, statups, this));
        final long starttime = System.currentTimeMillis();
        final CancelEffectAction cancelAction = new CancelEffectAction(applyto, this, starttime);
        final ScheduledFuture<?> schedule = BuffTimer.getInstance().schedule(cancelAction, starttime + 120000L - starttime);
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        ...
        if (infinity) {
            ...
            final CancelEffectAction cancelAction = new CancelEffectAction(applyto, this, starttime);
            final ScheduledFuture<?> schedule = BuffTimer.getInstance().schedule(cancelAction, starttime + this.duration - System.currentTimeMillis());
            this.statups = Collections.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.ENERGY_CHARGE, 10000));
            applyto.registerEffect(this, starttime, schedule, stat);
            this.statups = stat;
        }
    }
    
    private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, int newDuration) {
        int localDuration = newDuration;
        if (localDuration == 1000) {
            localDuration = (newDuration = 65535000);
        }
        if (primary) {
        ...
        switch (this.sourceid) {
            ...
            case 5220011: {
                if (applyto.getLinkMid() > 0) {
                    ...
                    applyto.getClient().getSession().write(MaplePacketCreator.giveHoming(this.sourceid, applyto.getLinkMid()));
                    normal = false;
                    break;
                }
                return;
            ...
            default: {
                ...
                if (this.isMonsterRiding()) {
                    ...
                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        applyto.getClient().getSession().write(MaplePacketCreator.giveMount(applyto, mountid2, this.sourceid, stat2));
                    ...
                    final int mountid = parseMountInfoA(applyto, this.sourceid, applyto.getId());
                    if (mountid != 0) {
    ...
    private int calcHPChange(final MapleCharacter applyfrom, final boolean primary) {
        ...
        if (this.hp != 0) {
            if (!this.skill) {
                ...
                if (this.isHeal()) {
                    hpchange += (int)Math.ceil(this.hp / 100.0 * applyfrom.getStat().getCurrentMaxHp());
                }
                else {
