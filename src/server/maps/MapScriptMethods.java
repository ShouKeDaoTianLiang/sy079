    ...
    public static void startScript_FirstUser(final MapleClient c, final String scriptName) {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case StageMsg_romio: {
                switch (c.getPlayer().getMapId()) {
                    case 926100000: {
                        c.getPlayer().getMap().startMapEffect("请在每一本书上面点点看,直到找到门为止！！", 5120021);
                        break;
                    }
                    case 926100001: {
                    ...
                    case 926100100: {
                        c.getPlayer().getMap().startMapEffect("充满能量的烧杯,请牢记必须一个一个丢，一个消失才丢下一个！", 5120021);
                        break;
                    }
                    case 926100200: {
                        c.getPlayer().getMap().startMapEffect("打怪，获取[蒙特鸠的实验资料],交给NPC过关！", 5120021);
                        break;
                    }
                    case 926100203: {
                    ...
                    case 926100401: {
                        c.getPlayer().getMap().startMapEffect("一个人打怪,然后引怪物到角落打你,使得朱丽叶不受攻击,然后其他人消灭BOSS！", 5120021);
                        break;
                    }
                }
                break;
