...
public class MapleMapFactory
{
    ...
    private int channel;
    private static final MapleMapFactory instance;
    private String[] mapBlock;
    
    private MapleMapFactory() {
    ...
    public static MapleMapFactory getInstance() {
        return MapleMapFactory.instance;
    }
    ...
    public final MapleMap getMap(final int mapid, final boolean respawns, final boolean npcs, final boolean reactors) {
        ...
        MapleMap map = (server.maps.MapleMap)(server.maps.MapleMap)this.maps.get(omapid);
        if (map == null) {
            this.lock.lock();
            try {
                ...
                map = (server.maps.MapleMap)(server.maps.MapleMap)this.maps.get(omapid);
                if (map != null) {
                    return map;
                }
                ...
                final List<AbstractLoadedMapleLife> custom = (java.util.List)(java.util.List)MapleMapFactory.customLife.get(mapid);
                if (custom != null) {
                ...
                map.setCreateMobInterval((short)MapleDataTool.getInt(mapData.getChildByPath("info/createMobInterval"), ServerConstants.怪物刷新时间 * 1000));
    ...
    public static int loadCustomLife() {
        ...
        try {
            ...
            final PreparedStatement ps = con.prepareStatement("SELECT * FROM `spawns`");
            ...
            while (rs.next()) {
                ...
                final List<AbstractLoadedMapleLife> entries = (java.util.List)(java.util.List)MapleMapFactory.customLife.get(mapid);
    ...
    private void addAreaBossSpawn(final MapleMap map) {
        ...
        int mobtime = -1;
        String msg = null;
        final boolean shouldSpawn = true;
        final boolean sendWorldMsg = false;
        Point pos1 = null;
        Point pos2 = null;
        Point pos3 = null;
        switch (map.getId()) {
            ...
            case 101030404: {
                mobtime = 1200;
                monsterid = 3220000;
                msg = "这里的气息感觉阴森森的..有种莫名的奇怪感觉.,似乎有什么奇怪的东西出现了。";
            ...
            case 250010304: {
                mobtime = 3600;
                monsterid = 7220000;
                msg = "这里的气息感觉阴森森的..有种莫名的感奇怪觉.,似乎有什么奇怪的东西出现了。";
    ...
    private MapleNodes loadNodes(final int mapid, final MapleData mapData) {
        MapleNodes nodeInfo = (server.maps.MapleNodes)(server.maps.MapleNodes)MapleMapFactory.mapInfos.get(mapid);
        if (nodeInfo == null) {
    ...
    public String[] initMapBlock() {
        if (this.mapBlock != null) {
            return this.mapBlock;
        }
        final Properties dbProp = new Properties();
        try {
            final FileInputStream is = new FileInputStream("CashPack.txt");
            dbProp.load(is);
            is.close();
        }
        catch (IOException ex) {
            System.out.println("无法加载 CashPack.txt 的商城信息数据文件.");
        }
        return this.mapBlock = dbProp.getProperty("mapBlock").split(",");
    }
