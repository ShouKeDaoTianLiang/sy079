...
public final class MapleMap
{
    ...
    public MapleMap(final int mapid, final int channel, final int returnMapId, final float monsterRate) {
        ...
        this.runningOid = new AtomicInteger(100000);
        this.runningOidLock = new ReentrantLock(true);
        ...
        final EnumMap objsMap = new EnumMap(MapleMapObjectType.class);
        final EnumMap objlockmap = new EnumMap(MapleMapObjectType.class);
    ...
    public void addMapObject(final MapleMapObject mapobject) {
        ...
        final ReentrantReadWriteLock rl = (java.util.concurrent.locks.ReentrantReadWriteLock)(java.util.concurrent.locks.ReentrantReadWriteLock)this.mapobjectlocks.get(mapobject.getType());
        rl.writeLock().lock();
        try {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (de.itemId == 0) {
                ...
                if (mesos > 0) {
                    this.spawnMobMesoDrop((int)(mesos * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate), this.calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
                    mesoDropped = true;
                }
            }
            else {
        ...
        final int mesos2 = Randomizer.nextInt(mob.getLevel()) + mob.getLevel();
        if (mesos2 > 0) {
            final double lastMeso = (chr.getStat().realMesoBuff - 100.0 <= 0.0) ? 100.0 : (chr.getStat().realMesoBuff - 100.0);
            this.spawnMobMesoDrop((int)(mesos2 * (lastMeso / 100.0) * chr.getDropMod() * cmServerrate), this.calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
            mesoDropped = true;
        }
        ...
        final int cashModifier = (int)(mob.getStats().isBoss() ? 0L : (mob.getMobExp() / 1000L + mob.getMobMaxHp() / 10000L));
            ...
            if (Randomizer.nextInt(999999) < de2.chance && (de2.continent < 0 || (de2.continent < 10 && this.mapid / 100000000 == de2.continent) || (de2.continent < 100 && this.mapid / 10000000 == de2.continent) || (de2.continent < 1000 && this.mapid / 1000000 == de2.continent))) {
                ...
                if (de2.itemId == 0) {
                    ...
                    if (GameConstants.getInventoryType(de2.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        idrop = new Item(de2.itemId, (short)0, (short)((de2.Maximum != 1) ? (Randomizer.nextInt(de2.Maximum - de2.Minimum) + de2.Minimum) : 1), (short)0);
                    }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (mobid == 8810018 && this.mapid == 240060200) {
            Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无数次的挑战，" + chr.getName() + "所带领的队伍终于击败了暗黑龙王的远征队！你们才是龙之林的真正英雄~").getBytes());
            FileoutputUtil.log("日志/Logs/Log_暗黑龙王.rtf", this.MapDebug_Log());
            ...
            Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无数次的挑战，" + chr.getName() + "所带领的队伍终于击败了混沌暗黑龙王的远征队！你们才是龙之林的真正英雄~").getBytes());
            FileoutputUtil.log("日志/Logs/Log_暗黑龙王.rtf", this.MapDebug_Log());
            if (this.speedRunStart > 0L) {
            ...
            Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, chr.getName() + "经过带领的队伍经过无数次的挑战，终于击败了时间的宠儿－品克缤的远征队！你们才是时间神殿的真正英雄~").getBytes());
            if (this.speedRunStart > 0L) {
            ...
            FileoutputUtil.log("日志/Logs/Log_品克缤.rtf", this.MapDebug_Log());
        }
        else if (mobid == 8800002 && this.mapid == 280030000) {
            FileoutputUtil.log("日志/Logs/Log_扎昆.rtf", this.MapDebug_Log());
            if (this.speedRunStart > 0L) {
        ...
        if (ServerConstants.get怪物掉落抵用卷()) {
            final int 抵用卷 = Randomizer.nextInt(3);
            chr.modifyCSPoints(2, 抵用卷);
            chr.dropMessage(5, "你从怪物身上获得[" + 抵用卷 + "]抵用券");
            chr.UpdateCash();
        }
        if (ServerConstants.get怪物掉落点卷()) {
            final int 点卷 = Randomizer.nextInt(3);
            chr.modifyCSPoints(1, 点卷);
            chr.dropMessage(5, "你从怪物身上获得[" + 点卷 + "]点券");
            chr.UpdateCash();
        }
        if (monster.getId() == chr.get怪物ID() && chr.get怪物数量() > 0) {
            chr.gain怪物数量(-1);
            chr.getClient().getSession().write(MaplePacketCreator.sendHint("任务进度提示：\r\n#b还需要消灭#r" + chr.get怪物数量() + "#b只！", 200, 200));
        }
        else if (monster.getId() == chr.get怪物ID() && chr.get怪物数量() == 0) {
            chr.getClient().getSession().write(MaplePacketCreator.sendHint("任务进度提示：\r\n#b消灭#r#o" + chr.get怪物数量() + "##b已完成！", 200, 200));
        }
        if (withDrops) {
    ...
    public void setLootable(final boolean loot) {
        this.lootable = loot;
    }
    ...
    public final boolean containsNPC(final int npcid) {
        ...
        try {
            for (final MapleMapObject obj : (this.mapobjects.get(MapleMapObjectType.NPC)).values()) {
            	MapleNPC n = (MapleNPC) obj;
            	if (n.getId() == npcid) {
                    return true;
                }
            }
    ...
    public MapleMonster getMonsterById(final int id) {
        ...
        try {
            ...
            for (final MapleMapObject obj  : (this.mapobjects.get(MapleMapObjectType.MONSTER)).values()) {
    ...
    public MapleReactor getReactorById(final int id) {
        ...
        try {
            ...
            for (final  MapleMapObject obj  : (this.mapobjects.get(MapleMapObjectType.REACTOR)).values()) {
            	MapleReactor n = (MapleReactor) obj;
    ...
    public final void removeNpc(final int npcid) {
        ...
        try {
            final Iterator itr = (this.mapobjects.get(MapleMapObjectType.NPC)).values().iterator();
            while (itr.hasNext()) {
                final MapleNPC npc = (server.life.MapleNPC)(server.life.MapleNPC)itr.next();
    ...
    public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final short questid) {
    ...
    public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final Item item, final Point pos, final boolean ffaDrop, final boolean playerDrop) {
        final Point droppos = this.calcDropPos(pos, pos);
        final MapleMapItem drop = new MapleMapItem(item, droppos, dropper, owner, (byte)2, playerDrop);
        if ((item.getItemId() >= 5000000 && item.getItemId() <= 5999999) || item.getUniqueId() > 0) {
            owner.dropMessage(1, "现金物品无法丢弃\r\n你的物品消失了");
            return;
        }
        ...
        try {
            for (final MapleMapObject o : (this.mapobjects.get(MapleMapObjectType.REACTOR)).values()) {
                ...
                if (react.getReactorType() == 100 && GameConstants.isCustomReactItem(react.getReactorId(), item.getItemId(), react.getReactItem().getLeft()) && react.getReactItem().getRight() == item.getQuantity() && react.getArea().contains(drop.getPosition()) && !react.isTimerActive()) {
                    MapTimer.getInstance().schedule(new ActivateItemReactor(drop, react, c), 1000L);
    ...
    public List<MapleCharacter> getAllPlayersThreadsafe() {
        ...
        (this.mapobjectlocks.get(MapleMapObjectType.PLAYER)).readLock().lock();
        try {
            ...
            (this.mapobjectlocks.get(MapleMapObjectType.PLAYER)).readLock().unlock();
        }
        return ret;
    }
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (this.hasBoat() == 2 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
    ...
    public List<MapleMapObject> getMapObjectsInRange(final Point from, final double rangeSq) {
        ...
        for (final MapleMapObjectType type : MapleMapObjectType.values()) {
            (this.mapobjectlocks.get(type)).readLock().lock();
            try {
                (this.mapobjects.get(type)).values().stream().filter(mmo -> from.distanceSq(mmo.getPosition()) <= rangeSq).forEach(ret::add);
            }
            finally {
                (this.mapobjectlocks.get(type)).readLock().unlock();
            }
    ...
    public final List<MapleCharacter> getPlayersInRectAndInList(final Rectangle box, final List<MapleCharacter> chrList) {
        ...
        return (List<MapleCharacter>)character;
    }
    ...
    public final List<MapleCharacter> getCharactersThreadsafe() {
        ...
        return (List<MapleCharacter>)chars;
    }
    ...
    public void AutoNx(final int dy) {
        for (final MapleCharacter chr : this.characters) {
            ...
            final int cashdy = 1 + Randomizer.nextInt(dy);
            ...
            chr.getClient().getSession().write(MaplePacketCreator.yellowChat("[系统奖励] 恭喜你挂机获得[" + dy + "]点抵用卷,[" + dy + "]点经验,[" + dy + "]"));
        }
    }
    
    public void AutoNxmht(int 抵用卷) {
        final int 等级2 = 0;
        final int 等级3 = 0;
        final int 等级4 = 0;
        final OtherSettings1 item_id = new OtherSettings1();
        final String[] getItempb_等级2 = item_id.getItempb_等级2();
        final String[] getItempb_等级3 = item_id.getItempb_等级3();
        final String[] getItempb_等级4 = item_id.getItempb_等级4();
        for (final MapleCharacter chr : this.characters) {
            final int 经验 = chr.getLevel() * Integer.parseInt(getItempb_等级2[等级2]);
            final int 豆豆 = Integer.parseInt(getItempb_等级3[等级3]);
            抵用卷 = Integer.parseInt(getItempb_等级4[等级4]);
            chr.gainExp(经验, true, false, true);
            chr.modifyCSPoints(1, 抵用卷);
            chr.gainBeans(豆豆);
            chr.dropTopMsg("您获得" + 经验 + "点经验值," + 豆豆 + "个豆豆." + 抵用卷 + "点点卷");
        }
    }
    
    public void respawn(final boolean force) {
        if (ServerConstants.复古刷怪) {
            this.lastSpawnTime = System.currentTimeMillis();
            if (force) {
        ...
        if (ServerConstants.加速怪物刷新模式) {
            this.lastSpawnTime = System.currentTimeMillis();
            if (force) {
                final int numShouldSpawn = this.monsterSpawn.size() * ServerConstants.monsterSpawn - this.spawnedMonstersOnMap.get();
                if (numShouldSpawn > 0) {
                    int spawned = 0;
                    for (final Spawns spawnPoint : this.monsterSpawn) {
                        spawnPoint.spawnMonster(this);
                ...
                final int numShouldSpawn2 = (ServerConstants.地图最大怪物数量 > 0) ? Math.max(defaultNum, ServerConstants.地图最大怪物数量) : defaultNum;
                if (numShouldSpawn2 > 0) {
                    int spawned2 = 0;
                    final List<Spawns> randomSpawn2 = new ArrayList<Spawns>(this.monsterSpawn);
                    Collections.shuffle(randomSpawn2);
                    for (final Spawns spawnPoint3 : randomSpawn2) {
                        if (!this.isSpawns && spawnPoint3.getMobTime() > 0) {
                            continue;
                        }
                        if (spawnPoint3.shouldSpawn() || GameConstants.isForceRespawn(this.mapid) || (this.monsterSpawn.size() < 10 && this.maxRegularSpawn > this.monsterSpawn.size() && this.partyBonusRate > 0)) {
                            spawnPoint3.spawnMonster(this);
                            ++spawned2;
                        }
                        if (spawned2 >= numShouldSpawn2) {
                            break;
                        }
                    }
                }
    ...
    public final boolean makeCarnivalReactor(final int team, final int num) {
        ...
        for (final Pair guard : this.nodes.getGuardians()) {
            if (((Number)guard.right).intValue() == team || ((Number)guard.right).intValue() == -1) {
                boolean found = false;
    ...
    public List<Integer> getAllUniqueMonsters() {
        ...
        try {
            for (final MapleMapObject mmo : (this.mapobjects.get(MapleMapObjectType.MONSTER)).values()) {
                final int theId = ((AbstractLoadedMapleLife)mmo).getId();
    ...
    public void spawnRabbit(int hp) {
        ...
        final OverrideMonsterStats overrideStats = new OverrideMonsterStats(hp, onemob.getMobMaxMp(), 0L, false);
    ...
    public final int mobCount() {
        final List mobsCount = this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.MONSTER));
        return mobsCount.size();
    }
    
    public final int playerCount() {
        final List players = this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.PLAYER));
        return players.size();
    }
    ...
    public void 房间重置() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000301, true);
            cserv.getMapFactory().HealMap(980000301);
        }
    }
    
    public void 房间重置1() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000101, true);
            cserv.getMapFactory().HealMap(980000101);
        }
    }
    
    public void 房间重置2() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000201, true);
            cserv.getMapFactory().HealMap(980000201);
        }
    }
    
    public void 房间重置3() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000401, true);
            cserv.getMapFactory().HealMap(980000401);
        }
    }
    
    public void 房间重置5() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000601, true);
            cserv.getMapFactory().HealMap(980000601);
        }
    }
    
    public void 房间重置4() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            cserv.getMapFactory().destroyMap(980000501, true);
            cserv.getMapFactory().HealMap(980000501);
        }
    }
    ...
    public void setPartyBonusRate(final int ii) {
