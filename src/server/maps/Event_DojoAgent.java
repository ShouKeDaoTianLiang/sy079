...
public class Event_DojoAgent
{
    ...
    public static void 道场任务失败(final MapleCharacter chr) {
        final MapleMap currentmap = chr.getMap();
        final MapleMap deadMap = chr.getClient().getChannelServer().getMapFactory().getMap(925020002);
        if (chr.getParty() != null && chr.getParty().getMembers().size() > 1) {
            for (final MaplePartyCharacter mem : chr.getParty().getMembers()) {
                final MapleCharacter partyChr = currentmap.getCharacterById(mem.getId());
                if (partyChr != null) {
