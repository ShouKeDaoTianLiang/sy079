...
public class NPCScriptManager extends AbstractScriptManager
{
    ...
    public final void start(final MapleClient c, final int npc, final int wh) {
        ...
        try {
            ...
            if (!this.cms.containsKey(c)) {
                ...
                c.getPlayer().dropMessage(5, "你现在已经假死");
            }
        }
        catch (Exception e) {
            System.err.println("NPC 脚本错误, 它ID为 : " + npc + "_" + wh + "." + e);
            if (c.getPlayer().isGM()) {
                c.getPlayer().dropMessage("[系統提示] NPC " + npc + "_" + wh + "脚本错误 " + e + "");
            }
            FileoutputUtil.log("日志/Logs/Log_Script_脚本异常.rtf", "Error executing NPC script, NPC ID : " + npc + "_" + wh + "." + e);
            this.dispose(c);
        }
        finally {
            lock.unlock();
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection, final int wh) {
        if (mode != -1) {
            final NPCConversationManager cm = (scripting.NPCConversationManager)(scripting.NPCConversationManager)this.cms.get(c);
            ...
            try {
                ...
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage("[系统提示] NPC " + cm.getNpc() + "_" + wh + "脚本错误 " + e + "");
                }
                System.err.println("NPC 脚本错误. 它ID為 : " + cm.getNpc() + "_" + wh + ":" + e);
                this.dispose(c);
                FileoutputUtil.log("日志/Logs/Log_Script_脚本异常.rtf", "Error executing NPC script, NPC ID : " + cm.getNpc() + "_" + wh + "." + e);
            }
            finally {
                lock.unlock();
            }
    ...
    public final void startQuest(final MapleClient c, final int npc, final int quest) {
        ...
        try {
            if (!this.cms.containsKey(c)) {
                ...
                c.getPlayer().setConversation(1);
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage("[系统提示]建立任务脚本:" + quest + "。");
                }
            ...
            FileoutputUtil.log("日志/Logs/Log_Script_脚本异常.rtf", "Error executing Quest script. (" + quest + ")..NPCID: " + npc + ":" + e);
            this.dispose(c);
        }
        finally {
            lock.unlock();
    ...
    public final void startQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            FileoutputUtil.log("日志/Logs/Log_Script_脚本异常.rtf", "Error executing Quest script. (" + cm.getQuest() + ")..NPCID: " + cm.getNpc() + ":" + e);
            this.dispose(c);
        }
        finally {
            lock.unlock();
    ...
    public final void dispose(final MapleClient c) {
        final NPCConversationManager npccm = (scripting.NPCConversationManager)(scripting.NPCConversationManager)this.cms.get(c);
        if (npccm != null) {
            this.cms.remove(c);
            if (npccm.getType() == -1) {
                if (npccm.getwh() == 0) {
