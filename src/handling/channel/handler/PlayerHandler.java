...
public class PlayerHandler
{
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (itemId == 3011000) {
            boolean haz = false;
            for (final Item item : c.getPlayer().getInventory(MapleInventoryType.CASH).list()) {
        ...
        if (GameConstants.isFishingMap(chr.getMapId()) && itemId == 3010184) {
            boolean haz = false;
            for (final Item item : c.getPlayer().getInventory(MapleInventoryType.CASH).list()) {
                if (item.getItemId() == 5340000) {
                    haz = true;
    ...
    public static final void TakeDamage(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (damage > 0) {
            ...
            if (attacker != null && bouncedam_A != null && damage > 0) {
                ...
                final int 抗压伤害 = (int)(抗压.getEffect(chr.getSkillLevel(抗压)).getDamage() / 100.0 * damage);
    ...
    public static final void UseItemEffect(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        final short flag = toUse.getFlag();
    ...
    public static final void closeRangeAttack(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        final WeakReference[] clones = chr.getClones();
    ...
    public static final void MagicDamage(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getMp() - beforeMp < effect.getMpCon() && c.getPlayer().getBuffedValue(MapleBuffStat.INFINITY) == null) {
            final int remainingMp = beforeMp - effect.getMpCon();
    ...
    public static final void DropMeso(final int meso, final MapleCharacter chr) {
        if (!chr.isAlive() || meso < 100 || meso > 50000 || meso > chr.getMeso()) {
    ...
    public static final void MovePlayer(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        slea.readLong();
        slea.readLong();
        final List<LifeMovementFragment> res = MovementParse.parseMovement(slea, 1, chr);
        c.getPlayer().setLastRes(res);
        if (res != null) {
            if (slea.available() != 18L) {
                System.err.println("slea.available != 18 (movement parsing error");
                return;
            }
            final MapleCharacter player = c.getPlayer();
            try {
                if (!player.isHidden()) {
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.movePlayer(chr.getId(), res), false);
                }
                MovementParse.updatePosition(res, player, 0);
                player.getMap().movePlayer(player, player.getPosition());
            }
            catch (Exception e) {
                System.err.println("Failed to move player (" + player.getName() + ")");
            }
        }
    }
