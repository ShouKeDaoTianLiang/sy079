    ...
    public static final void PlayerInteraction(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
            case 0: {
                ...
                if (!chr.getMap().getMapObjectsInRange(chr.getPosition(), 20000.0, Arrays.asList(MapleMapObjectType.SHOP, MapleMapObjectType.HIRED_MERCHANT)).isEmpty() && chr.getClient().getChannel() != 1) {
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    final Item shop = c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte)slea.readShort());
            ...
            case 4: {
                if (chr.getTrade() != null && chr.getTrade().getPartner() != null) {
                    if (Boolean.parseBoolean(ServerProperties.getProperty("channel.跨图交易")) && chr.getMapId() != chr.getTrade().getPartner().getChr().getMapId()) {
                        chr.dropMessage(1, "和你交易的玩家不在同一个地图。");
                        chr.getTrade().getPartner().getChr().getClient().getSession().write(MaplePacketCreator.getTradeCancel((byte)0, 0));
                        chr.getTrade().getPartner().getChr().setTrade(null);
                        chr.setTrade(null);
                        return;
                    }
            ...
            case 14: {
                ...
                final String[] itemgy_id = item_id.getItemgy_id();
                ...
                if (c.getPlayer().getGMLevel() == 1) {
                    c.getPlayer().dropMessage(1, "肥水不流外人田。所以你的东西不能交易给玩家。");
            ...
            case 31: {
                ...
                if (ivItem.getQuantity() >= bundles_perbundle) {
                    ...
                    if (ii2.isCash(ivItem.getItemId())) {
                        c.getPlayer().dropMessage(1, "现金道具不能给别人。");
                        c.sendPacket(MaplePacketCreator.enableActions());
                        return;
                    }
                    for (int j = 0; j < itemgy_id.length; ++j) {
                        if (ivItem.getItemId() == Integer.parseInt(itemgy_id[j])) {
                            c.getPlayer().dropMessage(1, "这个物品是禁止雇佣贩卖的.");
                            c.sendPacket(MaplePacketCreator.enableActions());
                            return;
                        }
            ...
            case 38: {
                ...
                for (int k = 0; k < imps.getItems().size(); ++k) {
                    if (imps.getItems().get(k).bundles == 0) {
                        imps.getItems().remove(k);
                    }
                }
