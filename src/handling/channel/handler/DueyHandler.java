...
public class DueyHandler
{
    ...
    public static final List<MapleDueyActions> loadItems(final MapleCharacter chr) {
        final List packages = new LinkedList();
        final Connection con = DatabaseConnection.getConnection();
        try {
            ...
            rs.close();
            ps.close();
            return (List<MapleDueyActions>)packages;
        }
        catch (SQLException se) {
            se.printStackTrace();
            return null;
    ...
    private static final MapleDueyActions getItemByPID(final int packageid) {
        try {
            final Map<Integer, Pair<Item, MapleInventoryType>>  iter = ItemLoader.DUEY.loadItems(false, packageid);
            if (iter != null && iter.size() > 0) {
                final Iterator<Pair<Item, MapleInventoryType>> i$ = iter.values().iterator();
                if (i$.hasNext()) {
                    final Pair<Item, MapleInventoryType> i = i$.next();
