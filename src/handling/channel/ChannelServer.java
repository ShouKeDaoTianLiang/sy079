...
public class ChannelServer implements Serializable
{
    ...
    public final void run_startup_configurations() {
        ...
        try {
            (this.init = new ServerConnection(ServerType.频道服务器, this.port, 0, this.channel)).run();
            this.eventSM.init();
            this.loadEvents();
        }
        catch (Exception e) {
    ...
    public static void startChannel_Main() {
        ...
        System.out.printf("服务器IP - " + ServerProperties.getProperty("tms.IP"), new Object[0]);
    }
    ...
    public void AutoNx(final int dy) {
        ...
        final String[] getItempb_等级1 = item_id.getItempb_等级1();
        for (int i = 0; i < getItempb_等级1.length; ++i) {
            this.mapFactory.getMap(Integer.parseInt(getItempb_等级1[i])).AutoNxmht(dy);
        }
    }
