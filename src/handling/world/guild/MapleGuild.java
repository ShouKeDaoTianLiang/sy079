...
public class MapleGuild implements Serializable
{
    ...
    public static void 伤害排行(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`, `ddj` FROM characters ORDER BY `ddj` DESC LIMIT 100");
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.伤害排行榜(npcid, rs));
            ps.close();
            rs.close();
        }
    ...
    public final void guildChat(final String name, final int cid, final String msg) {
        ...
        FileoutputUtil1.玩家谈话("" + name + "家族对话.txt", "玩家: " + name + " 在家族中说: " + msg + "\r\n时间：" + FileoutputUtil.CurrentReadable_Time() + "");
    }
    ...
    public final List<MapleBBSThread> getBBS() {
        ...
        Collections.sort(ret, new ThreadComparator());
        return ret;
    }
    ...
    public final void editBBSThread(final int localthreadid, final String title, final String text, final int icon, final int posterID, final int guildRank) {
        final MapleBBSThread thread = (handling.world.guild.MapleBBSThread)(handling.world.guild.MapleBBSThread)this.bbs.get(localthreadid);
    ...
    public static void displayLevelRanks(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`, `vip`, `level`, `meso` FROM characters ORDER BY `level` DESC LIMIT 150");
            final ResultSet rs = ps.executeQuery();
    ...
    public static void 转生排名(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`, `ddj`, `level`, `meso` FROM characters ORDER BY `ddj` DESC LIMIT 150");
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.转生(npcid, rs));
            ps.close();
            rs.close();
        }
        catch (SQLException ex) {}
    ...
    public static void 人气排行(final MapleClient c, final int npcid) {
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.showRQRanks(npcid, rs));
            ps.close();
            rs.close();
        }
        catch (SQLException ex) {}
