...
public class MapleClient implements Serializable
{
    ...
    private long loginDeadTime;
    private long loginLastTime;
    private int loginPassCount;
    private int logintimelcount;
    ...
    public void setLastActionId(final int actionId) {
        this.lastActionId = actionId;
    }
    
    public MapleClient(final MapleAESOFB send, final MapleAESOFB receive, final MapleSession session) {
        this.channel = 1;
        this.accId = 1;
        this.charslots = MapleClient.DEFAULT_CHARSLOT;
        this.loggedIn = false;
        ...
        this.deadtime = 1000L;
        this.lasttime = 0L;
        this.loginDeadTime = 5000L;
        this.loginLastTime = 0L;
        this.send = send;
    ...
    public void announce(final MaplePacket packet) {
    ...
    public void unlockAcc() {
        ...
        for (final MapleClient c : Client.getClients()) {
    ...
    public final void unLockDisconnect() {
        this.sendPacket(MaplePacketCreator.serverNotice(1, "您的帐号被他人登入。"));
    ...
    public final void disconnect(final boolean RemoveInChannelServer, final boolean fromCS, final boolean shutdown) {
        if (this.player != null && this.isLoggedIn()) {
            ...
            final IMaplePlayerShop ips = this.player.getPlayerShop();
            if (this.player.getTrade() != null) {
                MapleTrade.cancelTrade(this.player.getTrade(), this);
            }
            if (ips != null && (!ips.isAvailable() || (ips.isOwner(this.player) && ips.getShopType() != 1) || (ips.isOwner(this.player) && ips.getShopType() == 1 && !ips.isOpen()))) {
            ...
            if (!fromCS) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    FileoutputUtil.outputFileError("日志/Logs/Log_卡账号.rtf", e);
                ...
                if (ch2 > 0) {
                    final ChannelServer ch_ = ChannelServer.getInstance(ch2);
                    if (ch_ != null) {
                        final MapleCharacter chr = ch_.getPlayerStorage().getCharacterByName(this.player.getName());
    ...
    public final void DebugMessage(final StringBuilder sb) {
        ...
        sb.append(" || 连接状态: ");
        sb.append(this.getSession().getChannel().isActive());
        sb.append(" || 正在关闭: ");
        sb.append(this.getSession().getChannel().isOpen());
        sb.append(" || CLIENT: ");
        sb.append(this.getSession().getChannel().attr((AttributeKey)MapleClient.CLIENT_KEY) != null);
        sb.append(" || 是否已登陆: ");
    ...
    public int getCharacterSlots() {
        ...
        if (this.charslots != MapleClient.DEFAULT_CHARSLOT) {
    ...
    public boolean messageOn() {
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT message FROM accounts WHERE id = ?");
            ps.setInt(1, this.getAccID());
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public void setMessageToggle(final int x) {
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE accounts SET message = ? WHERE id = ?");
            ps.setInt(1, x);
            ps.setInt(2, this.getAccID());
            ps.executeUpdate();
            ps.close();
    ...
    public void set战力输出(final int x) {
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE accounts SET fwn = ? WHERE id = ?");
            ps.setInt(1, x);
            ps.setInt(2, this.getAccID());
            ps.executeUpdate();
            ps.close();
    ...
    public int getLogintimelcount() {
        return this.logintimelcount;
    }
    
    public void setLogintimelcount(final int logintimelcount) {
        this.logintimelcount = logintimelcount;
    }
    
    public long getLoginDeadTime() {
        return this.loginDeadTime;
    }
    
    public void setLoginDeadTime(final long loginDeadTime) {
        this.loginDeadTime = loginDeadTime;
    }
    ...
    public void setLoginLastTime(final long loginLastTime) {
        this.loginLastTime = loginLastTime;
    }
    
    public int getLoginPassCount() {
        return this.loginPassCount;
    }
    
    public void setLoginPassCount(final int loginPassCount) {
        this.loginPassCount = loginPassCount;
    }
    
    static {
        DEFAULT_CHARSLOT = LoginServer.getMaxCharacters();
        CLIENT_KEY = AttributeKey.newInstance(".CLIENT");
