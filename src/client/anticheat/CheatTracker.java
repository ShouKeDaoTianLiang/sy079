...
public class CheatTracker
{
    ...
    public final void checkMoveMonster(final Point pos, final MapleCharacter chr) {
        if (pos.equals(this.lastMonsterMove)) {
            ...
            if (this.monsterMoveCount > 50) {
                ...
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[管理员信息] 开挂玩家[" + MapleCharacterUtil.makeMapleReadable(chr.getName()) + "] 地图ID[" + chr.getMapId() + "] 怀疑使用吸怪! ").getBytes());
    ...
    public void checkSameDamage(final int dmg, final double expected) {
        if (dmg > 2000 && this.lastDamage == dmg && this.chr.get() != null && (this.chr.get().getLevel() < 180 || dmg > expected * 2.0)) {
            ++this.numSameDamage;
            if (this.numSameDamage > 5) {
                this.registerOffense(CheatingOffense.SAME_DAMAGE, this.numSameDamage + " times, 攻击伤害 " + dmg + ", 预期伤害 " + expected + " [等级: " + this.chr.get().getLevel() + ", 职业: " + this.chr.get().getJob() + "]");
                this.numSameDamage = 0;
            }
        }
        else {
        ...
        switch (offense) {
            case SAME_DAMAGE: {
                ...
                if (this.gm_message == 0) {
                    ...
                    Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[管理員訊息] 开挂玩家[" + MapleCharacterUtil.makeMapleReadable(chrhardref.getName()) + "] 地图ID[" + chrhardref.getMapId() + "] suspected of hacking! " + StringUtil.makeEnumHumanReadable(offense.name()) + ((param == null) ? "" : (" - " + param))).getBytes());
                    final String note = "时间：" + FileoutputUtil.CurrentReadable_Time() + " || 玩家名字：" + chrhardref.getName() + "|| 玩家地图：" + chrhardref.getMapId() + "|| 外挂类型：" + offense.name() + "\r\n";
