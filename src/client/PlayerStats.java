...
public class PlayerStats implements Serializable
{
    ...
    public void recalcLocalStats(final boolean first_login) {
        ...
        this.def = 100;
        for (final Item item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
            final Equip equip = (Equip)item;
            if (equip.getPosition() == -11 && GameConstants.isMagicWeapon(equip.getItemId())) {
                final Map eqstat = MapleItemInformationProvider.getInstance().getEquipStats(equip.getItemId());
            ...
            if (set > 0) {
                ...
                if (this.setHandling.get(set) != null) {
                    value += (Integer)(Integer)this.setHandling.get(set);
                }
        ...
        for (final Item item2 : chra.getInventory(MapleInventoryType.CASH)) {
        ...
        for (final Item item2 : chra.getInventory(MapleInventoryType.ETC)) {
    ...
    public double getDropBuff() {
        if (this.incRewardProp > 100.0) {
            this.incRewardProp = 100;
        }
        ...
        for (Equip eq : all) {
            ...
            if (eq.getEquipLevels() > lvlz) {
                for (int i = eq.getEquipLevels() - lvlz; i > 0; --i) {
                    final Map inc = ii.getEquipIncrements(eq.getItemId());
                    if (inc != null && inc.containsKey(lvlz + i)) {
                        eq = ii.levelUpEquip(eq, (java.util.Map)inc.get(lvlz + i));
                    }
    ...
    public boolean checkEquipDurabilitys(final MapleCharacter chr, final int gain) {
        ...
        for (final Equip eqq : all) {
            if (eqq.getDurability() == 0) {
                ...
                chr.getClient().sendPacket(MaplePacketCreator.modifyInventory(false, new ModifyInventory(1, eqq, pos)));
            }
            else {
